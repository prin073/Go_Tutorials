...
// creating a slice with make(to create slice of type map). we need to define initial size. which increases dynamically
...
func getFirstNames() []string {
	...
	for _, booking := range bookings {
		names := booking["firstName"] // from map
...
func bookTickets(userTickets uint, firstName string, lastName string, email string) (uint, []map[string]string) {
	...
	//map
	var userData = make(map[string]string) //make initializes an empty map

	userData["firstName"] = firstName
	userData["lastName"] = lastName
	userData["email"] = email

	//since map is defined as string, string type hence we need to convert userTickets value to str to store in the defined map
